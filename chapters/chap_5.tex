...
For this chapter, the only type that \(A\) can adopt is that of \(\unit\). Therefore, resource-aware types have the form \(\ratype{\unit}{p_0}{p_1}\).
...
Let us now perform an exemplary type derivation using the type rules we just introduced, to get a better understanding of how the definitions orchestrate together. For this, consider the following program:
...
Having a solid grasp of how the type rules orchestrate together, having exercised a simple example, we now prove soundness for the \emph{let-tick} programming language.
...
\begin{theorem}[Soundness of typing for let-tick language]\label{thm:soundness-let-tick}
