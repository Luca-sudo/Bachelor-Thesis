...
\subsection{Equivalence and Capacity Relation}

Composing functions lies at the very core of functional programming languages. Our programming language does not offer an explicit handle to compose functions, for simplicity. We can, however, emulate function composition using let expressions. For this, consider two function with the following signatures:
\[
   f :: \rafunc{\ralist{2}{\typeint}}{\ralist{}{\typeint}}{}{}
   \qquad
   g :: \rafunc{\ralist{3}{\typeint}}{\ralist{}{\typeint}}{}{}
\]

Emulating function composition can be achieved using a let expression, like so:
\[
   \letexp{y}{f~x}{g~y}
\]

As we have introduced lists, along with their potential, into the programming language, we would face inconsistencies when chaining together computations that have potentials assigned to them. In the example above, we have no means of composing potentials assigned to lists. As a result, the composite function would not account for the potential of the function \(g\).

To enhance the type rule for let expressions, such that potentials also compose, we need to introduce two new relations:

\begin{enumerate}[label=\Roman*.]
   \item Equivalence relation, which ensures that the set of values represented by two types is the same.
   \item Capacity relation, which allows us to compare potentials in a meaningful and ergonomic way.
\end{enumerate}

\begin{definition}[Equivalent Types]
   Let \(A, B\) be types. Two types are \emph{equivalent}, if the set of values, that inhabit the respective type, is the same. 
   \[
      A \typeequiv B :\iff A \text{ is equivalent to } B
   \]
\end{definition}

\begin{definition}[Capacity Relation]
   Let \(A, B\) be types. We define a \emph{capacity relation} on types, by comparing their potential:
   \[
      A \capacity B :\iff \Phi(v : A) \geq \Phi(w : B)
   \]
\end{definition}
\todo{Maybe use different symbol?}

We will use both of these definitions in the rule \href{def:t-let-6}{(T:Let)}, to ensure that let expression with potentials can be typed properly.
