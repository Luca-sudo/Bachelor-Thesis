...
\[
   ...
   {E(l) = (v_x, v_{xs}) \qquad \evals{\envaugment{E}{l}{(v_x, v_{xs})}}{e_2}{v}{Q(\cons{x}{xs})}{Q'(\cons{x}{xs})} \qquad \evals{E}{e_1}{v_1}{q_0}{q_1}}
...
To properly evaluate recursive functions we introduce an evaluation rule for function application. We define \(e_f\) as the body of the function \(f\), and the variable name \(y_i\) as the \(i\)-th argument used in the definition of \(f\). 
